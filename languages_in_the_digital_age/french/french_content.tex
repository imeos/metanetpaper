...
{\em Fondation Alliance Française} is an organisation whose mission is to
...
organise national dictation competitions (see also the highly complex
...
century. Written French tends to nominalise rather than verbalise
...
international organisations. However, with globalisation, the
...
realisation. But language also has aspects that are shared between
...
recognise any word) to 10\% being comparable to the one required to go
...
A key issue for future research is the {\bf personalisation} of interactive
systems. To some degree, this is already possible, for example in
speech transcription systems, which can also recognise the genre, age,
accent or identity ({\bf diarisation}) of the person who is speaking, and in
...
to synthesise an answer from a set of documents that could contain
...
components, for instance: {\bf text summarisation} and {\bf text
  generation}. Summarisation, obviously, refers to the task of making
...
to actually synthesise new sentences, i.\,e., to build a summary of
...
information extraction, and summarisation have since the 1990s been
...
other resources for most of these tasks. Summarisation systems, when
...
been limited to the surface realisation modules (the ``generation
...
(corpus, lexica, dictionaries, etc.) for French and the organisation
...
(Digitisation platform, Social impact media monitoring, Personalised
...
digitisation effort for national document assets. This area can
...
them are affiliated to a large research organisation (CNRS, INRIA
...
\begin{tabular}{>{\columncolor{orange1}}p{.50\linewidth}@{\hspace*{6mm}}c@{\hspace*{6mm}}c@{\hspace*{6mm}}c@{\hspace*{6mm}}c@{\hspace*{6mm}}c@{\hspace*{6mm}}c@{\hspace*{6mm}}c}
 ...
 % &\begin{sideways}\makecell[l]{\makecell[l]{Disponibilité} }\end{sideways} 
  ...
  Tokenisation, POS tagging, morphology analysis/generation &4&4&4&4&4&3&3 \\ \addlinespace
  ...
  Summarisation, Question Answering, Advanced Technologies for Information Access &3&3&3&3&3&2&2\\ \addlinespace
...
\item Many of the resources lack standardisation, i.\,e., even if they
  exist, sustainability is not guaranteed; concerted programs and
  initiatives are needed to standardise data and interchange
