...
%
%
%            - META-NET Language Whitepaper | Finnish paper -
...
\author{
  ...
  Pirkko Nuolijärvi, Kotimaisten kielten tutkimuskeskus \\
  Lauri Carlson, Helsingin yliopisto \\
  Martti Vainio, Helsingin yliopisto \\
  Antti Arppe, Helsingin yliopisto \\
  Aino Piehl, Kotimaisten kielten tutkimuskeskus \\
  Mietta Lennes, Helsingin yliopisto \\
  Hanna Westerlund, Helsingin yliopisto \\
  Mirka Hyvärinen, Helsingin yliopisto
  \\ --- \\
  Prof. Dr. Kimmo Koskenniemi, University of Helsinki \\
  Dr. Krister Lindén, University of Helsinki \\
  Prof. Dr. Pirkko Nuolijärvi,
  Research Institute for the Languages of Finland \\
  Prof. Dr. Lauri Carlson, University of Helsinki \\
  Dr. Martti Vainio, University of Helsinki \\
  Dr. Antti Arppe, University of Helsinki \\
  Aino Piehl, MA, Research Institute for the Languages of Finland \\
  Mietta Lennes, MA, University of Helsinki \\
  Hanna Westerlund, MA, University of Helsinki \\
  Mirka Hyvärinen, University of Helsinki
}
...
\clearpage
  ...
  % --------------------------------------------------------------------------
  \ssection{Tiivistelmä --- Executive Summary}
...
\clearpage
  ...
  % --------------------------------------------------------------------------
  \ssection{Uhka kansalliskielille on haaste kieliteknologialle --- Risk for Our Languages and a Challenge for Language Technology}
...
\ssubsection{Kielten väliset rajat esteenä Euroopan tietoyhteiskunnan kehitykselle --- Language Borders Hinder the European Information Society}
...
\ssubsection{Kielet kohtaavat uusia uhkia --- Our Languages at Risk}
...
Euroopan noin 60 kieltä muodostavat yhden sen rikkaimmista ja
...
Europe’s approximately 60 languages are one of its richest and most
...
\ssubsection{Kieliteknologia tukee kielten säilymistä --- Language Technology is a Key Enabling Technology}
...
\ssubsection{Kieliteknologian mahdollisuuksia --- Opportunities for Language Technology}
...
\ssubsection{Kieliteknologian haasteita --- Challenges Facing Language Technology}
...
\ssubsection{Kielen omaksumisesta --- Language Acquisition in Humans and Machines}
...
\clearpage
  ...
  % --------------------------------------------------------------------------
  \ssection{Suomen kieli Euroopan tietoyhteiskunnassa --- Finnish in the European Information Society}
...
\ssubsection{Perustietoa suomen kielen asemasta ja käytöstä --- General Facts}
...
\ssubsection{Suomen kielen erityispiirteitä --- Particularities of the Finnish Language}
...
\ssubsection{Suomen kielen kehityksestä --- Recent Developments}
...
\ssubsection{Suomen kielen huolto --- Language Cultivation in Finland}
...
\ssubsection{Kieli ja oppiminen --- Language in Education}
...
\ssubsection{Kansainvälisiä näkökulmia --- International Aspects}
...
\ssubsection{Suomen kieli ja Internet --- Finnish on the Internet}
...
\clearpage
  ...
  % --------------------------------------------------------------------------
  \ssection{Kieliteknologian suomen kielen tuki --- Language Technology Support for Finnish}
...
Oikealla olevassa kuvassa esitetään kieliteknologian kentän
...
expressed in. The figure on the right illustrates the LT
...
interactiontechnology, and machine translation. This includes
...
\ssubsection{Sovellusarkkitehtuurit --- Application Architectures}

\ParallelLText{\selectlanguage{finnish}
Kielenkäsittelyn sovellusohjelmat koostuvat tavallisesti useista
komponenteista, jotka kuvastavat kielen eri ominaisuuksia. Oikealla
oleva kuva esittää tyypillisen tekstinkäsittelyn arkkitehtuurin
...
Figure~X shows a highly simplified architecture that can be found in a
...
\ssubsection{Keskeiset sovellusalat --- Core Application Areas}
...
\ssubsubsection{Kielentarkistus --- Language Checking}
...
kehittyneempiä.  Kielikohtaisten \underline{tekstin analyysin}
...
tekstikorpus. Tähän asti tilastolliset mallit on enimmäkseen kehitetty
...
language-dependent algorithms for \underline{text analysis},
they detect errors
...
\ssubsubsection{Hakukoneet --- Web Search}
...
laajempaa kielitieteellistä näkemystä tekstin tulkinnassa. Kokeilut,
joissa on hyödynnetty leksikaalisia resursseja kuten koneluettavat
...
\ssubsubsection{Puheteknologia --- Speech Interaction Technology}
...
Speech interaction technology is used to create interfaces that enable users to
interact in spoken language instead of a graphical display, keyboard and mouse.
Today, these voice user interfaces (VUI) are usually used for partially or
...
\begin{enumerate}
\item Automatic speech recognition (ASR) determines which words are actually
...
\ssubsubsection{Konekääntäminen --- Machine Translation}
...
\ssubsection{Muut sovellusalat --- Other Application Areas}
...
\ssubsection{Kieliteknologian opetus Suomessa --- Educational Programmes}
...
\ssubsection{Kansalliset hankkeet --- National Projects and Efforts}
...
\ssubsection{Kieliteknologiset työkalut ja kieliaineistot --- Availability of Tools and Resources}

\ParallelLText{\selectlanguage{finnish}
Alla olevassa taulukossa esitetään kieliteknologisten resurssien
...
\ParallelRText{\selectlanguage{english}
The following table summarises the current state of language technology support
for the Finnish language. The rating for existing tools and resources is based
on the estimates ofwas generated by leading experts in the field who provided
...
& \rotatebox{90}{Määrä} \rotatebox{90}{Quantity}
& \rotatebox{90}{Saatavuus} \rotatebox{90}{Availability}
& \rotatebox{90}{Laatu} \rotatebox{90}{Quality}
& \rotatebox{90}{Kattavuus} \rotatebox{90}{Coverage}
& \rotatebox{90}{Valmiusaste} \rotatebox{90}{Maturity}
& \rotatebox{90}{Vakaus} \rotatebox{90}{Sustainability}
& \rotatebox{90}{Soveltuvuus} \rotatebox{90}{Adaptability} \\ \hline
\multicolumn{8}{l|}
{Kieliteknologiat (työkalut, teknologiat ja sovellukset)}           \\
\multicolumn{8}{l|}
{Language Technology (Tools, Technologies and Applications)} \\ \hline
...
\parbox{4cm}{Kieliopit \strut \\ \strut
Grammars}                &  2   & 5   & 4   & 4   & 4   & 3   & 3   \\ \hline
\end{tabular}
\end{center}
\ParallelLText{\selectlanguage{finnish}
...
\ssubsection{Kieltenvälistä vertailua --- Cross-language comparison}

\ParallelLText{\selectlanguage{finnish}
Kieliteknologisten sovellusten saatavuus vaihtelee merkittävästi
kielten välillä. Kieltenvälistä vertailua varten tässä osiossa
esitellään yhteenveto arvioista, joiden kohteena on kaksi
sovellusalaa, konekääntäminen ja puheenkäsittely, sekä yksi taustalla
palveleva teknologia, tekstin analyysiin.  Lisäksi arvioidaan
kieliteknologian sovellusten tuotekehityksen tarvitsemien resurssien
saatavuutta.
}

\ParallelRText{\selectlanguage{english}
The current state of LT support varies considerably from one language community
to another. In order to compare the situation between languages, this section
will present an evaluation based on two sample application areas (machine
translation and speech processing) and one underlying technology (text
analysis), as well as basic resources needed for building LT applications.
}
...
\end{center}
\ParallelLText{\selectlanguage{finnish}
Puheenkäsittelyn ja klusterien kuvaukset:
\begin{itemize}
\item Klusteri 1 (erinomainen kieliteknologian tuki): On saatavissa
    teknologioita, joiden laatu ja suorituskyky ovat erinomaisia ja joita
    voidaan hyödyntää kaikissa relevanteissa sovelluksissa.

\item Klusteri 2 (hyvä tuki): On saatavissa teknologioita, joiden laatu ja
    suorituskyky ovat hyviä ja jotka ovat käyttökelpoisia joissakin tietyissä
    sovelluksissa tai joillakin sovellusaloilla.

\item Klusteri 3 (kohtuullinen tuki): On olemassa tutkimusprototyyppejä,
    ensimmäisiä kaupallisia sovelluksia tai vapaasti saatavissa olevia
    palveluita, joiden laatu ja suorituskyky vaihtelevat.

\item Klusteri 4 (alhainen tai lähes olematon tuki): On olemassa sovelluksia
    suunnitteluasteella tai ensimmäisen asteen prototyyppejä, joiden laatu ja
    suorituskyky ovat hyvin rajoitettuja.
\end{itemize}
}

\ParallelRText{\selectlanguage{english}
Cluster descriptions for Speech Processing and Machine Translation:
\begin{itemize}
\item Cluster 1 (excellent LT support): Technologies with excellent quality and
    performance exist that can be used in practically all relevant
    applications.

\item Cluster 2 (good support): Technologies with reasonable quality and
    performance exist whose usability is limited to some applications or
    domains.

\item Cluster 3 (medium support): Research prototypes, first commercial
    applications or free services exist with varying quality and performance.

\item Cluster 4 (low to almost no support): From the drawing board to rudimentary
    prototypes with very limited quality and performance.
\end{itemize}
}
...
&
   \begin{center}
German, Hungarian, Swedish, French, Dutch, Czech
   \end{center}
&
   \begin{center}
Spanish, Norwegian, Portuguese, Danish, Polish, Romanian, Bulgarian, Catalan,
Basque, Galician, Estonian
   \end{center}
&
   \begin{center}
Italian, Finnish, Croatian, Slovene, Icelandic, Irish, Greek, Latvian, Slovak,
Serbian, Lithuanian, Maltese
   \end{center}
\end{tabular}

Kuva 4: Kieliresurssien klusterit \\
Figure 4: Language clusters for Resources
\end{center}
\ParallelLText{\selectlanguage{finnish}
Tekstin analyysiohjelmien ja kieliresurssien klustereiden
kuvaukset:
\begin{itemize}
\item Klusteri 1 (erinomainen kieliteknologian tuki): On olemassa teknologioita
    tai resursseja, jotka ovat laajalti käytössä ja kattavat käytännöllisesti
    katsoen kaikki kielen ilmiöt (sanasto, yhdyssanat, kielioppi, metaforat
    jne.)

\item Klusteri 2 (erittäin hyvä tuki): On olemassa teknologioita tai
    resursseja,
    joita käytetään laajalti sovelluksissa ja jotka kattavat kielen
    oleellisimmat ilmiöt.

\item Klusteri 3 (hyvä tuki): On olemassa teknologioita tai resursseja, jotka
    kattavat kohtuullisen osan kielen kielitieteellisistä ilmiöistä ja jotka
    ovat käytössä tavallisesti erikoisaloja varten tehdyissä sovelluksissa.

\item Klusteri 4 (kohtuullinen tuki): On olemassa tutkimusprototyyppejä,
    joiden
    laatu ja suorituskyky vaihtelevat.

\item Klusteri 5 (alhainen tai lähes olematon tuki): On olemassa sovelluksia
    suunnitteluasteella tai ensimmäisen asteen prototyyppejä, joiden laatu ja
    suorituskyky ovat rajoitettuja.
\end{itemize}
...
\ParallelRText{\selectlanguage{english}
Cluster descriptions for Text Analysis and Resources:
\begin{itemize}
\item Cluster 1 (excellent LT support): Technologies/resources exist that are in
    widespread use and cover practically all linguistic phenomena (vocabulary,
    compounds, grammar, metaphors etc.) of a language.

\item Cluster 2 (very good support): Technologies/resources exist that are used
    in a variety of applications and cover the most important linguistic
    phenomena.

\item Cluster 3 (good support): Technologies/resources exist that cover a
    reasonable amount of linguistic phenomena and are used in applications that
    are usually domain-specific.

\item Cluster 4 (medium support): Research prototypes/resources exist, but
    quality and coverage varies.

\item Cluster 5 (low to almost no support): From the drawing board to rudimentary
    prototypes (very limited quality and coverage, toy systems)
\end{itemize}
...
\ssubsection{Johtopäätökset --- Conclusions}
...
\clearpage
  ...
  % --------------------------------------------------------------------------
  \ssection{META-NET --- About META-NET}
...
\ssubsection{Toimintalinjat --- Lines of Action}
...
\ssubsection{Jäsenet --- Member Organisations}
...
unused left head: Comparison continued
unused left head: Resources continued
unused left head: References
unused left head: Comparison continued more
unused left text: References
unused rite head: Comparison continued
unused rite head: Resources continued
unused rite head: References
unused rite head: Comparison continued more
unused rite text: References
