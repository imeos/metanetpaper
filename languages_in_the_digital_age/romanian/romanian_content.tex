...
\boxtext{Limba română are cinci litere cu diacritice: \textit{ă}, \textit{î}, \textit{â},\\[.3mm] \textit{ș}, \textit{ț}. Pentru ultimele două au circulat două variante: una cu virgulă sub literă, alta cu sedilă, însă numai prima variantă este recomandată.}
...
\boxtext{Există în străinătate peste 70 de centre în care\\ se predă limba română ca limbă străină\\ de către cadre didactice din învățământul\\[.3mm] universitar românesc.}
...
Tehnologiile limbajului sunt un domeniu de cercetare de sine stătător, cu o bogată literatură de specialitate. Cititorul interesat este invitat să consulte cărțile fundamentale ale domeniului, precum \cite{carstensen-etal1,jurafsky-martin01,manning-schuetze1,lt-world1,lt-survey1}.
...
\begin{quote}
\textit{Neam cumpărat un calculator care sa defectat dea doua zi: supt multe cuvinte se pune o linie roșă care nu pot cum sos cot.}
\end{quote}
...
Două teze de doctorat, însoțite de mai multe articole științifice și susținute de diferite proiecte naționale sau internaționale, precum STAR și ACCURAT, sunt dedicate acestui domeniu \cite{tufisMT,irimia}. 
%\vspace*{-0.09 cm}
...
ceea ce este destul de greu de înțeles dacă nu există nici o explicație despre cine este \textit{ea} sau \textit{el} (din cliticul \textit{-l} se înțelege doar că există o persoană atacată care este de genul masculin).
...
\item []Era un pod \underline{peste} rau.\\
  `There was a bridge \underline{over} the river.'
\end{itemize}
\end{example}
...
\item []Polițistul \underline{a văzut} omul \underline{cu telescopul}.\\
  `The policeman \underline{saw} the man \underline{with the telescope}.'
\end{itemize}
\end{example}
...
\item []Polițistul a văzut \underline{omul} \underline{cu arma}.\\
  `The policeman saw \underline{the man} \underline{with the gun}.`
\end{itemize}
\end{example}
