...
%
%
%            - META-NET Language Whitepaper | Romanian content -
% 
% ----------------------------------------------------------------------------
...
\bsection*{Cuprins --- Table of Contents}
...
\boxtext{Tehnologiile limbajului ajută la \newline unificarea Europei.}
...
\boxtext{Suntem martorii unei revoluții digitale\newline comparabile cu inventarea tiparului\newline de către Gutenberg.}
...
\boxtext{Ritmul actual al progresului tehnologic\newline este prea lent.}
...
\boxtext{Oamenii achiziționează competențe lingvistice în două moduri diferite: învățând exemple și învățând regulile \newline care stau la baza limbii.}
...
\boxtext{Limba română are cinci litere cu diacritice: \textit{ă}, \textit{î}, \textit{â}, \textit{ș}, \textit{ț}. Pentru ultimele două au circulat două variante:  \newline una cu virgulă sub literă, alta cu sedilă, \newline însă numai prima variantă este recomandată.}
...
Această secțiune se concentrează asupra instrumentelor și resurselor TL celor mai importante și oferă o imagine de ansamblu a activităților legate de TL din România și Republica Moldova. Instrumentele și resursele care sunt \underline{subliniate} în text pot fi regăsite, de asemenea, în tabelul de la sfârșitul acestui capitol.
...
\caption{Corector de limbă (sus: statistic, jos: bazat pe reguli)}
...
\boxtext{Tehnologia vorbirii reprezintă punctul de plecare pentru crearea de interfețe care să permită utilizatorului să interacționeze utilizând limbajul vorbit și nu o interfață grafică, o tastatură ori un mouse.}
...
Campanii de evaluare sunt folosite pentru compararea sistemelor de traducere automată, a diferitelor abordări și a situației existente pentru diferite limbi. Figura~\ref{fig:euromatrix_ro}, prezentată în cadrul proiectului european Euromatrix+, arată performanțele obținute de sisteme la traducerile automate încrucișate în 22 din cele 23 de limbi oficiale ale Uniunii Europene (limba irlandeză nu a fost comparată), raportate la scorul BLEU \cite{bleu1}, unde un scor mai mare indică o traducere mai bună. Un traducător uman ar avea un scor de aprox. 80 puncte.
...
During the migration of Slavic tribes over the territory of nowadays Romania, the language underwent a process of transformation in all its compartments: phonetics, vocabulary, morphology and syntax. However, morphology, the backbone of a language, remained Latin in most of its aspects. The Cyrillic alphabet was adopted in this period, especially due to the church influence. The old Slavonic was the liturgical language of the Romanian Orthodox Church until the late 18th century, when Romanian started a process of re-latinization, modernization and westernization. It is now when many words of other origin are replaced by Latin words, borrowed directly or indirectly, via other Romance languages (French and Italian). French as a language of culture in the last 2 centuries and France as a place where the Romanian aristocracy sent their children to school justify the existence of extremely numerous words of this origin in Romanian. Lately, English took the place of French and Romanian has many Anglicisms, entirely, partially or at all adapted to its phonetic and morphologic systems. 
...
\boxtext{Romanian has five letters using diacritics: \textit{ă}, \textit{î}, \textit{â}, \textit{ș}, \textit{ț}. For the last two, a couple of variants have circulated: one with a comma under the letter, and another one with a cedilla. However, only the former is recommended.}
...
\boxtext{There are over 70 international centres abroad where Romanian is taught as \newline a foreign language by Romanian \newline university teachers.}
...
\boxtext{The Consortium for the Digitalization of Romanian Language – ConsILR – organizes annually a scientific workshop dedicated to research in LT regarding the Romanian language.}
...
Software applications for language processing typically consist of several components that mirror different aspects of language. While such applications are typically very complex, figure~\ref{fig:textprocessingarch_en} shows a highly simplified architecture of a typical text processing system. The first three modules handle the structure and meaning of the input text:
...
Now that data is increasingly found in non-textual formats, there is a need for services that deliver multimedia information retrieval by searching images, audio files and video data. In the case of audio and video files, a speech recognition module must convert the speech content into text (or into a phonetic representation) that can be matched against a user query afterwards.
...
At RACAI, a part-of-speech tagger able to identify the lemma (dictionary form) and the part of speech of words in texts is available as a web service \cite{webservicesUrl}. For instance, if the user’s query for a web search contains \textit{evenimente} (events), the root (or lemmatized form) of the word can be used instead for search, i.e. \textit{eveniment} (event) \cite{webservicesArt}. 
